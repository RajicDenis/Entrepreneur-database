...
<div class="col-md-6 col-md-offset-3 mb">
	...
	<div class="showbox">
		...
		<div class="bc-left">
			...
			<h4 style="text-align: center;">({{ $company->contact }})</h2>
		</div>

		<div class="bc-right">
			<div class="info">{{ $company->address }}<span class="glyphicon glyphicon-home hiden"></span></div>
			<div class="info">{{ $company->email }}<span class="glyphicon glyphicon-envelope hiden"></span></div>
			<div class="info">{{ $company->phone }}<span class="glyphicon glyphicon-phone-alt hiden"></span></div>
			<div class="info">{{ $company->role }}<span class="glyphicon glyphicon-globe hiden"></span></div>
			<div class="info">{{ $company->mb }}<span class="glyphicon glyphicon-asterisk hiden"></span></div>
		</div>

		<a class="edit-btn" href="{{ action('CompanyController@edit', $company->id) }}"><span class="glyphicon glyphicon-edit"></span></a>
	</div>
				  ...
				  <tbody>
					...
					@foreach($company->contacts as $c)
					    <tr>
					      ...
					      <td style="width: 25px;"><a href="{{ action('ContactController@editContact', $c->id) }}" class="btn btn-warning btn-xs">Uredi</a></td>
					    </tr>
					   @php $e++; @endphp
					@endforeach
				  </tbody>
			...
			<div class="panel-heading">
		  	...
		  	<div class="panel-body">
		    	<table class="table table-striped">
				  ...
				  <tbody>
					@php $e = 1; @endphp
					@foreach($company->interests as $int)
					    <tr>
					      <td>{{ $e }}</td>
					      <td>{{ $int->name }}</td>
					      <td>{{ $int->area }}</td>
					      <td style="width: 25px;"><a class="del" href="{{ action('InterestController@removeCompanyInterest', $int->id) }}"><span class="glyphicon glyphicon-remove"></span></a></td>
					    </tr>
					   @php $e++; @endphp
					@endforeach
				  </tbody>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-horizontal" action="{{ action('ContactController@addNew') }}" role="form" method="POST">				
...
<div class="modal fade" id="newInterest" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-horizontal" action="{{ action('InterestController@addCompanyInterest') }}" role="form" method="POST">				
			<input type="hidden" name="_token" value="{{ csrf_token() }}">	
			<input type="hidden" name="cid" value="{{ $company->id }}">	
